...
func New(origArgs []string) (*Options, error) {
	...
	fs.DurationVar(&opts.BindingOptionsTimeout, "binding-options-timeout", config.DefaultBindingOptionsTimeout, "Timeout for input binding subscription discovery requests to the app (HTTP OPTIONS or gRPC ListInputBindings). Increase for apps with slow startup (e.g. JVM/JIT workloads). Non-positive values use the default.")
