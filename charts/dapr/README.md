...
| `dapr_placement.cluster.forceInMemoryLog`      | Use in-memory log store and disable volume attach. When `false` (the default), a fresh install requires a PVC for the raft log, so a cluster without a default StorageClass must either set this to `true` or set `dapr_placement.volumeclaims.storageClassName`. On `helm upgrade`, the chart detects the live StatefulSet via `lookup` and keeps the claim template absent if the release never had one, because `StatefulSet.spec.volumeClaimTemplates` is immutable in Kubernetes and changing it would fail the upgrade. | `false`                 |
| `dapr_placement.cluster.logStorePath`          | Mount path for persistent volume for log store in unix-like system                                                                                                        | `/var/run/dapr/raft-log` |
| `dapr_placement.cluster.logStoreWinPath`       | Mount path for persistent volume for log store in windows                                                                                                                 | `C:\\raft-log`          |
