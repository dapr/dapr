...
| `global.scheduler.placement.experimental`          | Serves actor placement from the Scheduler service instead of the standalone placement service. When "true", the placement StatefulSet is not deployed. Enable only once every sidecar runs a Dapr version which supports it. | `false` |
