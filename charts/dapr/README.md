...
| `global.trustDistribution.enabled`            | Distribute the trust anchors as a ConfigMap into every namespace so daprd sidecars hot reload trust anchor changes from a mounted file. Forced off when `global.rbac.namespaced` is true                | `true` |
| `global.trustDistribution.configMapName`      | Name of the per-namespace ConfigMap holding the distributed trust anchors                                                                                                                               | `dapr-root-ca.crt` |
...
| `global.scheduler.placement.enabled`          | Serves actor placement from the Scheduler service instead of the standalone placement service. When "true", the placement StatefulSet is not deployed. Toggling in either direction requires no sidecar restarts. Enable only once every sidecar runs a Dapr version which supports it. | `false` |
...
| `dapr_placement.disseminateCoalesceWindow` | If `>0`, after a dissemination round completes, defer the next round by this window so additional host registers/disconnects arriving inside the window collapse into a single follow-up round. The first event in an idle (REPORT) state still fires immediately, so cold-starts and isolated churn stay responsive. Default `0` disables coalescing and is the right choice for most clusters. Set to `100ms`-`250ms` only if you observe round thrashing during bulk events (eg. 100+ replica scale-ups or rolling restarts), where it collapses what would otherwise be O(N) follow-up rounds into O(1). | `0s` |
...
| `dapr_scheduler.securityContext.fsGroup`      | Integer value for `securityContext.fsGroup`. Adds the Scheduler process to the file system group that owns the mounted database volume, which block volumes (e.g. AWS EBS) require for a non-root process to write. Set it to `null` on platforms that assign an fsGroup themselves from a namespace range (e.g. OpenShift) so none is rendered.        | `65532`                                    |
...
| `dapr_scheduler.etcdCompactionMode`           | Compaction mode for etcd. Can be 'periodic' or 'revision'. Tuned for high write/watch workloads (workflows): bounded MVCC history is more predictable than wall-clock retention, and keeps watch-from-revision replays cheap.                                                                                                                         | `revision`                                 |
| `dapr_scheduler.etcdCompactionRetention`      | Compaction retention for etcd. Time string (e.g. `10m`) when `etcdCompactionMode=periodic`; number of revisions when `etcdCompactionMode=revision`.                                                                                                                                                                                                  | `1000000`                                  |
| `dapr_scheduler.etcdSnapshotCount`            | Number of committed transactions to trigger a snapshot to disk. Raised from etcd's 10k default for high-write workloads (workflows) to reduce snapshot/WAL-truncation overhead.                                                                                                                                                                      | `100000`                                   |
...
| `dapr_scheduler.etcdBackendBatchLimit`        | Maximum operations before committing the backend transaction. Matches etcd's default; lowering it increases fsync frequency at the cost of write throughput.                                                                                                                                                                                         | `10000`                                    |
| `dapr_scheduler.etcdBackendBatchInterval`     | Maximum time before committing the backend transaction. Matches etcd's default; lowering it shrinks the post-crash data-loss window at the cost of write throughput.                                                                                                                                                                                 | `100ms`                                    |
| `dapr_scheduler.etcdMaxTxnOps`                | Maximum number of operations permitted in a single etcd transaction. Workflow fan-out can batch many puts/deletes into one txn, so this is raised well above etcd's default of 128.                                                                                                                                                                  | `10000`                                    |
...
| `dapr_scheduler.overrideBroadcastHostPort`    | When set, overrides the host:port that the scheduler broadcasts to clients. Useful when running external daprd instances that connect through a TLS gateway, where the gateway address (rather than the pod IP) must be advertised so that clients route through the gateway and receive correct SNI-based TLS routing. Unset by default.             | `""`                                       |
...
| `dapr_sentry.caTTL`                 | Time-to-live used when generating or renewing the root and issuer certificates                                                                          | `8760h`                 |
| `dapr_sentry.caRenewal.enabled`     | Automatic append-only CA renewal. Defaults to enabled, except when user-provided certificates are set (`tls.issuer` / `tls.root`), where renewal migrates the cluster to a sentry-generated CA and so must be opted into explicitly | `""`                    |
| `dapr_sentry.caRenewal.threshold`   | Fraction of the issuer certificate's lifetime after which the CA is automatically renewed, in the range (0, 1)                                          | `0.9`                   |
| `dapr_sentry.caRenewal.propagationGrace` | How long sentry keeps signing with the old issuer key after appending a renewed trust anchor, giving the new anchor time to propagate everywhere   | `24h`                   |
...
| `dapr_sidecar_injector.allowedServiceAccounts`            | Comma-separated list of extra allowed service accounts in `namespace:name` format. Supports glob patterns using Go's [`path.Match`](https://pkg.go.dev/path#Match) syntax: `*` matches any sequence of characters, `?` matches a single character, and `[...]` matches a character class. Examples: `my-ns:my-sa` (exact match), `my-ns:*` (all service accounts in a namespace), `team-*:deploy-*` (prefix matching). | `""` |
| `dapr_sidecar_injector.allowedServiceAccountsPrefixNames` | **Deprecated.** Use `allowedServiceAccounts` with glob patterns instead (e.g., `ns1*:sa2*`). This value still functions but logs a deprecation warning. Comma-separated list of extra allowed service accounts in `namespace:serviceaccount` format with trailing-asterisk prefix matching. | `""`                                                                                                                                                                                                                         |
