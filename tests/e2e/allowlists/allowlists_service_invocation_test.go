...
var allowListsForServiceInvocationTests = []struct {
	...
	},
	{
		...
		"allowlists-callee-http",
		"opDeny",
		fmt.Sprintf("failed to invoke, id: allowlists-callee-http, err: rpc error: code = PermissionDenied desc = access control policy has denied access to id: spiffe://public/ns/%s/%s operation: opDeny verb: POST", kube.DaprTestNamespace, kube.FormatAppID("allowlists-caller")),
		"http",
		403,
	},
	{
	...
	},
	{
		...
		"allowlists-callee-grpc",
		"httptogrpctest",
		fmt.Sprintf("HTTP call failed with failed to invoke, id: allowlists-callee-grpc, err: rpc error: code = PermissionDenied desc = access control policy has denied access to id: spiffe://public/ns/%s/%s operation: httpToGrpcTest verb: NONE", kube.DaprTestNamespace, kube.FormatAppID("allowlists-caller")),
		"grpc",
		403,
	},
}
