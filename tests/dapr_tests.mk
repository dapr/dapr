...
test-e2e-all: check-e2e-env
	...
	# test
	GOOS=$(TARGET_OS_LOCAL) DAPR_TEST_NAMESPACE=$(DAPR_TEST_NAMESPACE) DAPR_TEST_TAG=$(DAPR_TEST_TAG) DAPR_TEST_REGISTRY=$(DAPR_TEST_REGISTRY) DAPR_TEST_MINIKUBE_IP=$(MINIKUBE_NODE_IP) go test -p 1 -count=1 -v -tags=e2e ./tests/e2e/...
...
setup-test-env-kafka:
	...
	kubectl wait --timeout=10m --for=condition=complete job `kubectl get jobs  -n $(DAPR_TEST_NAMESPACE) | awk '/dapr-kafka-config/ {print $$1}'` -n $(DAPR_TEST_NAMESPACE)
...
setup-test-components:
	...
	$(KUBECTL) apply -f ./tests/config/kubernetes_secret_2.yaml --namespace $(DAPR_TEST_NAMESPACE)
