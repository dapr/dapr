...
func buildDaprAnnotations(appDesc AppDescription) map[string]string {
	...
	if appDesc.DaprEnabled {
		annotationObject = map[string]string{
			...
			"dapr.io/app-id":                            FormatAppName(appDesc.AppName), // Use formatted name to isolate parallel tests
	...
	if len(appDesc.DisableInitEndpoints) != 0 {
		annotationObject["dapr.io/disable-init-endpoints"] = strings.Join(appDesc.DisableInitEndpoints, ",")
	}
