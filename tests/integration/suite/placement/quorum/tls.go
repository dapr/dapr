...
// tls tests placement can find quorum with tls enabled.
type tls struct {
	places []*placement.Placement
	sentry *sentry.Sentry
}

func (s *tls) Setup(t *testing.T) []framework.Option {
	...
	opts := []placement.Option{
		...
		placement.WithSentryAddress("localhost:" + strconv.Itoa(s.sentry.Port())),
	}
	s.places = []*placement.Placement{
	...
	return []framework.Option{
		framework.WithProcesses(s.sentry, s.places[0], s.places[1], s.places[2]),
	}
}

func (s *tls) Run(t *testing.T, ctx context.Context) {
	s.sentry.WaitUntilRunning(t, ctx)
	s.places[0].WaitUntilRunning(t, ctx)
	s.places[1].WaitUntilRunning(t, ctx)
	s.places[2].WaitUntilRunning(t, ctx)
	...
	secProv, err := security.New(ctx, security.Options{
		...
		TrustAnchors:            s.sentry.CABundle().TrustAnchors,
		AppID:                   "app-1",
		MTLSEnabled:             true,
	})
	...
	require.Eventually(t, func() bool {
		...
		host := "localhost:" + strconv.Itoa(s.places[i].Port())
