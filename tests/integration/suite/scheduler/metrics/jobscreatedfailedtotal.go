...
func init() {
	suite.Register(new(jobscreatedfailedtotal))
}

type jobscreatedfailedtotal struct {
	scheduler *scheduler.Scheduler
}

func (j *jobscreatedfailedtotal) Setup(t *testing.T) []framework.Option {
...
func (j *jobscreatedfailedtotal) Run(t *testing.T, ctx context.Context) {
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		total, ok := metrics.Metrics["dapr_scheduler_jobs_created_failed_total"]
		if !assert.True(c, ok) {
			return
		}
		assert.InDelta(c, 3.0, total["type=job"], 0.01)
		assert.InDelta(c, 2.0, total["type=actor"], 0.01)
