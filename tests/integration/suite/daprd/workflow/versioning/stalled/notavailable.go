...
func init() {
	suite.Register(new(versionnotavailable))
}

type versionnotavailable struct {
	workflow *workflow.Workflow
}

func (d *versionnotavailable) Setup(t *testing.T) []framework.Option {
...
func (d *versionnotavailable) Run(t *testing.T, ctx context.Context) {
	...
	// None of the workflows should have run
	require.False(t, runv1.Load())
	require.False(t, runv2.Load())
}
