...
func (cf *childfailed) Run(t *testing.T, ctx context.Context) {
	...
	cf.workflow.Registry().AddOrchestratorN("dedup-failparent", func(ctx *task.OrchestrationContext) (any, error) {
		err := ctx.CallSubOrchestrator("dedup-failchild").Await(nil)
		if err == nil {
			return nil, errors.New("child unexpectedly succeeded")
		}
		return nil, ctx.WaitForSingleEvent("done", time.Minute).Await(nil)
	})
	cf.workflow.Registry().AddOrchestratorN("dedup-failchild", func(ctx *task.OrchestrationContext) (any, error) {
		childCalls.Add(1)
		return nil, errors.New("planned failure")
	})

	cl := cf.workflow.BackendClient(t, ctx)
	id, err := cl.ScheduleNewOrchestration(ctx, "dedup-failparent")
