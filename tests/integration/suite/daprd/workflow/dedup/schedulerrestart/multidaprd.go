...
func (m *multidaprd) Run(t *testing.T, ctx context.Context) {
	...
	for i := range daprds {
		idx := i
		require.NoError(t, m.workflow.RegistryN(idx).AddOrchestratorN("wf",
			func(ctx *task.OrchestrationContext) (any, error) {
	...
	for i := range daprds {
		...
		id, err := cl.ScheduleNewOrchestration(ctx, "wf",
	...
	for i := range daprds {
		cl := m.workflow.BackendClientN(t, ctx, i)
		meta, err := cl.WaitForOrchestrationCompletion(ctx, ids[i])
