...
func (cc *childcompleted) Run(t *testing.T, ctx context.Context) {
	...
	cc.workflow.Registry().AddOrchestratorN("dedup-childparent", func(ctx *task.OrchestrationContext) (any, error) {
		if err := ctx.CallSubOrchestrator("dedup-child").Await(nil); err != nil {
			return nil, err
		}
		return nil, ctx.WaitForSingleEvent("done", time.Minute).Await(nil)
	})
	cc.workflow.Registry().AddOrchestratorN("dedup-child", func(ctx *task.OrchestrationContext) (any, error) {
		childCalls.Add(1)
		return "ok", nil
	})

	cl := cc.workflow.BackendClient(t, ctx)
	id, err := cl.ScheduleNewOrchestration(ctx, "dedup-childparent")
	require.NoError(t, err)
	_, err = cl.WaitForOrchestrationStart(ctx, id)
	require.NoError(t, err)
