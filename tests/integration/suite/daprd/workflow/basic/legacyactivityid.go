...
// legacyactivityid delivers an activity under the
// "<instanceID>::<taskID>::<generation>" actor ID shape by invoking the
// activity actor directly with a bare TaskScheduled event, and checks the
// result reaches the parent instance. The real dispatch is parked so only the
// planted delivery can complete the workflow.
...
func (l *legacyactivityid) Run(t *testing.T, ctx context.Context) {
	...
	legacyActorID := fmt.Sprintf("%s::%d::0", wfID, scheduled.GetEventId())
