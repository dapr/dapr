...
// previous generation consumed are not re-delivered to the generation that
// is persisted, and that this generation still runs. A leaked event would
// ride the kept-events carryover to the final generation and satisfy its
// wait before the test raises the event again.
...
func (a *abandoned) Run(t *testing.T, ctx context.Context) {
	...
	// The final generation is committed together with whatever it consumed
	// in its first turn: a re-delivered "go" would have completed it already.
