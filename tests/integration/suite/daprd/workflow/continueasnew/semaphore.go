...
func (s *semaphore) Run(t *testing.T, ctx context.Context) {
	...
	require.NoError(t, reg.AddOrchestratorN("sem-coordinator", func(ctx *task.OrchestrationContext) (any, error) {
		var st coordState
	...
	coordID, err := client.ScheduleNewOrchestration(ctx, "sem-coordinator",
		api.WithInstanceID("sem-coord"),
		api.WithInput(coordState{}),
	)
	require.NoError(t, err)
	_, err = client.WaitForOrchestrationStart(ctx, coordID)
	require.NoError(t, err)
	...
	meta, err := client.WaitForOrchestrationCompletion(ctx, coordID)
	require.NoError(t, err)
