...
func (c *carryover) Run(t *testing.T, ctx context.Context) {
	...
	t.Run("with keep preserves buffered events", func(t *testing.T) {
		...
		reg.AddWorkflowN("can-keep-ev", func(ctx *task.WorkflowContext) (any, error) {
			n := iteration.Add(1)
			if n == 1 {
		...
		id, err := client.ScheduleNewWorkflow(ctx, "can-keep-ev",
	...
	t.Run("keep preserves multiple event types", func(t *testing.T) {
		...
		reg.AddWorkflowN("can-multi-ev", func(ctx *task.WorkflowContext) (any, error) {
			var st state
		...
		id, err := client.ScheduleNewWorkflow(ctx, "can-multi-ev",
	...
	t.Run("keep with activity in same iteration", func(t *testing.T) {
		...
		reg.AddWorkflowN("can-keep-act", func(ctx *task.WorkflowContext) (any, error) {
			var inc int
		...
		id, err := client.ScheduleNewWorkflow(ctx, "can-keep-act",
	...
	t.Run("multiple CAN cycles preserve events across all", func(t *testing.T) {
		...
		reg.AddWorkflowN("can-multi-cycle", func(ctx *task.WorkflowContext) (any, error) {
			var st state
			require.NoError(t, ctx.GetInput(&st))
		...
		id, err := client.ScheduleNewWorkflow(ctx, "can-multi-cycle",
