...
func init() {
	suite.Register(new(timerdowngrade))
}

// timerdowngrade kills the master daprd while a workflow timer is pending and
// expects the release daprd to fire the timer reminder and complete the
// workflow.
type timerdowngrade struct {
	upgrade *wfupgrade.Upgrade
}

func (m *timerdowngrade) Setup(t *testing.T) []framework.Option {
	m.upgrade = wfupgrade.New(t, wfupgrade.WithMasterToLegacy())
...
func (m *timerdowngrade) Run(t *testing.T, ctx context.Context) {
