...
func init() {
	suite.Register(new(activitydowngrade))
}

// activitydowngrade kills the master daprd while an activity is executing and
// expects the release daprd to re-fire the durable run-activity reminder and
// complete the workflow.
type activitydowngrade struct {
	upgrade *wfupgrade.Upgrade
}

func (a *activitydowngrade) Setup(t *testing.T) []framework.Option {
	a.upgrade = wfupgrade.New(t, wfupgrade.WithMasterToLegacy())
...
func (a *activitydowngrade) Run(t *testing.T, ctx context.Context) {
