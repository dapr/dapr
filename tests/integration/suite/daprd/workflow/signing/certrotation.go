...
// certrotation verifies that workflow signing continues to work correctly when
// the signing certificate rotates (e.g. after a daprd restart where sentry
// issues a new SVID). The signature chain should remain valid with multiple
// certificate entries in the certificate table.
...
func (c *certrotation) Setup(t *testing.T) []framework.Option {
	...
	c.sched = scheduler.New(t, scheduler.WithSentry(c.sentry))
...
func (c *certrotation) Run(t *testing.T, ctx context.Context) {
	...
	reg.AddWorkflowN("sign-certrotate", func(ctx *dworkflow.WorkflowContext) (any, error) {
		var payload string
	...
	id, err := client.ScheduleWorkflow(ctx, "sign-certrotate")
	...
	certCountBefore := c.db.CountStateKeys(t, ctx, "sigcert")
	require.Positive(t, certCountBefore)
	...
	require.NoError(t, client.RaiseEvent(ctx, id, "continue",
		dworkflow.WithEventPayload("after-rotation")))

	_, err = client.WaitForWorkflowCompletion(ctx, id)
	require.NoError(t, err)

	certCountAfter := c.db.CountStateKeys(t, ctx, "sigcert")
	assert.Greater(t, certCountAfter, certCountBefore)
