...
// tampered verifies that when workflow history data is tampered with in the
// state store, the workflow is marked as FAILED on the next load and the
// failure details indicate a signature verification error.
...
func (t *tampered) Run(tt *testing.T, ctx context.Context) {
