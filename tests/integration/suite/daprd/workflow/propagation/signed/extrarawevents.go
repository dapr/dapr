...
func init() {
	suite.Register(new(extrarawevents)) //nolint:gochecknoinits
}

type extrarawevents struct {
	workflow *procworkflow.Workflow

	childInstanceID atomic.Value // string
}

func (s *extrarawevents) Setup(t *testing.T) []framework.Option {
...
func (s *extrarawevents) Run(t *testing.T, ctx context.Context) {
	s.workflow.WaitUntilRunning(t, ctx)

	app0Reg := s.workflow.Registry()
	app1Reg := s.workflow.RegistryN(1)

	app1Reg.AddWorkflowN("extraRawChild", func(ctx *task.WorkflowContext) (any, error) {
	...
	app0Reg.AddWorkflowN("extraRawParent", func(ctx *task.WorkflowContext) (any, error) {
		var out string
		return out, ctx.CallChildWorkflow("extraRawChild",
	...
	_, err := client0.ScheduleNewWorkflow(ctx, "extraRawParent")
	...
	// Append a bogus rawEvent. The chunk's signature was over the
	// original count of entries; adding any entry shifts EventsDigest
	...
	require.NotEmpty(t, ph.GetChunks()[0].GetRawEvents(),
		"sanity: chunk should have rawEvents before tampering")
	ph.GetChunks()[0].RawEvents = append(ph.GetChunks()[0].GetRawEvents(), []byte("bogus-extra-event-bytes"))
