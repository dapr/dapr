...
func (d *depth) Run(t *testing.T, ctx context.Context) {
	...
	d.workflow.Registry().AddWorkflowN("Workflow0", func(ctx *task.WorkflowContext) (any, error) {
		var result0 string
		...
		var result1 string
		err = ctx.CallChildWorkflow("Workflow1", task.WithChildWorkflowAppID(d.workflow.DaprN(1).AppID())).Await(&result1)
		if err != nil {
	...
	d.workflow.RegistryN(1).AddWorkflowN("Workflow1", func(ctx *task.WorkflowContext) (any, error) {
		var result1 string
		...
		var result2 string
		err = ctx.CallChildWorkflow("Workflow2", task.WithChildWorkflowAppID(d.workflow.DaprN(2).AppID())).Await(&result2)
		if err != nil {
	...
	d.workflow.RegistryN(2).AddWorkflowN("Workflow2", func(ctx *task.WorkflowContext) (any, error) {
		var result2 string
	...
	id, err := client0.ScheduleNewWorkflow(ctx, "Workflow0", api.WithInput("Hello from app1"))
