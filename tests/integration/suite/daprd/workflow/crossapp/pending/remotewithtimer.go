...
func init() {
	suite.Register(new(remotewithtimer))
}

type remotewithtimer struct {
	daprd1 *daprd.Daprd
	daprd2 *daprd.Daprd
...
func (r *remotewithtimer) Setup(t *testing.T) []framework.Option {
	...
	r.registry1.AddOrchestratorN("TimerThenRemoteWorkflow", func(ctx *task.OrchestrationContext) (any, error) {
		var input string
		...
		if err := ctx.CreateTimer(time.Second).Await(nil); err != nil {
			return nil, fmt.Errorf("timer failed: %w", err)
		}
...
func (r *remotewithtimer) Run(t *testing.T, ctx context.Context) {
	...
	id, err := client1.ScheduleNewOrchestration(ctx, "TimerThenRemoteWorkflow", api.WithInput("hello"))
