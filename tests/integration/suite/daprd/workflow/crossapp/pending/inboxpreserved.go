...
// inboxpreserved verifies that after the pre-save transitions the workflow to
// RUNNING, subsequent no-op replay executions (where all inbox events are
// duplicates and the orchestrator yields with 0 actions) do NOT clear the
// inbox.
...
func (p *inboxpreserved) Setup(t *testing.T) []framework.Option {
	...
	p.registry1.AddOrchestratorN("InboxPreservedWorkflow", func(ctx *task.OrchestrationContext) (any, error) {
		...
		var result string
		if err := ctx.CallActivity("RemoteActivity",
			task.WithActivityInput(input),
			task.WithActivityAppID(p.daprd2.AppID())).
	...
	p.registry2.AddActivityN("RemoteActivity", func(ctx task.ActivityContext) (any, error) {
		var input string
		if err := ctx.GetInput(&input); err != nil {
			return nil, err
		}
...
func (p *inboxpreserved) Run(t *testing.T, ctx context.Context) {
	...
	// Verify the inbox entry still exists in the state store. The key format is:
	// {actorType}||{actorID}||inbox-000000
