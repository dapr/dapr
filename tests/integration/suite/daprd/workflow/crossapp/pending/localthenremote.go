...
func (l *localthenremote) Setup(t *testing.T) []framework.Option {
	...
	l.registry1.AddOrchestratorN("LocalThenRemoteWorkflow", func(ctx *task.OrchestrationContext) (any, error) {
		var input string
		if err := ctx.GetInput(&input); err != nil {
			return nil, err
		}

		// Schedule both activities without awaiting so both TaskScheduled events
		// are generated in the same execution batch. This exercises the selective
		// filtering: local TaskScheduled stays in history while remote
		// TaskScheduled is excluded for retry.
		localTask := ctx.CallActivity("LocalActivity",
			task.WithActivityInput(input))
		remoteTask := ctx.CallActivity("RemoteActivity",
			task.WithActivityInput(input),
			task.WithActivityAppID(l.daprd2.AppID()))
	...
	l.registry1.AddActivityN("LocalActivity", func(ctx task.ActivityContext) (any, error) {
		l.localCallCount.Add(1)
		var input string
		if err := ctx.GetInput(&input); err != nil {
			return nil, err
		}
	...
	l.registry2.AddActivityN("RemoteActivity", func(ctx task.ActivityContext) (any, error) {
		var input string
		if err := ctx.GetInput(&input); err != nil {
			return nil, err
		}
...
func (l *localthenremote) Run(t *testing.T, ctx context.Context) {
	...
	id, err := client1.ScheduleNewOrchestration(ctx, "LocalThenRemoteWorkflow", api.WithInput("hello"))
	...
	assert.Equal(t, int32(1), l.localCallCount.Load(), "local activity should be called exactly once")
}
