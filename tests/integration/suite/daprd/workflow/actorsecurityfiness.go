...
func init() {
	suite.Register(new(actorsecurityfiness))
}

type actorsecurityfiness struct {
	daprd1 *daprd.Daprd
	daprd2 *daprd.Daprd
...
func (s *actorsecurityfiness) Setup(t *testing.T) []framework.Option {
...
func (s *actorsecurityfiness) Run(t *testing.T, ctx context.Context) {
	...
	// TEST 2: Can App 2 create its own workflow with same instance ID even tho it has nothing in its registry
	// this SHOULD Faillll I think cassie but it doesnt
	_, err = client2.ScheduleNewOrchestration(ctx, "pauser", api.WithInstanceID("pauser"))
	require.NoError(t, err, "App 2 can create workflow with same instance ID")

	// TEST 3: Direct actor call test - try to call App 1's actor from App 2
	// Try to call App 1's workflow actor directly
	actorType := fmt.Sprintf("dapr.internal.%s.%s.workflow", s.daprd1.Namespace(), s.daprd1.AppID())
	fmt.Printf("Trying to call actor type: %s\n", actorType)
	_, err = s.daprd2.GRPCClient(t, ctx).InvokeActor(ctx, &rtv1.InvokeActorRequest{
		ActorType: actorType,
		ActorId:   "pauser", //s.daprd1.AppID(),
