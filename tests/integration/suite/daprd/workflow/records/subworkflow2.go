...
func init() {
	suite.Register(new(subworkflow2))
}

type subworkflow2 struct {
	workflow *workflow.Workflow
}

func (a *subworkflow2) Setup(t *testing.T) []framework.Option {
...
func (a *subworkflow2) Run(t *testing.T, ctx context.Context) {
	...
	if a.workflow.Signing() {
		// Signing adds 9 rows: parent gets 1 sigcert, 3 signatures and
		// 1 ext-sigcert shared by both child completion attestations; each
		// child gets 1 sigcert and 1 signature.
		expected = 30
	}
