...
func init() {
	suite.Register(new(fanoutthreecluster))
}

type fanoutthreecluster struct {
...
func (f *fanoutthreecluster) Setup(t *testing.T) []framework.Option {
...
func (f *fanoutthreecluster) Run(t *testing.T, ctx context.Context) {
	...
	id1, err := client1.ScheduleNewOrchestration(ctx, "foo")
	require.NoError(t, err)
	id2, err := client2.ScheduleNewOrchestration(ctx, "foo")
	require.NoError(t, err)
	id3, err := client3.ScheduleNewOrchestration(ctx, "foo")
	require.NoError(t, err)

	_, err = client1.WaitForOrchestrationCompletion(ctx, id1)
	require.NoError(t, err)
	_, err = client2.WaitForOrchestrationCompletion(ctx, id2)
	require.NoError(t, err)
	_, err = client3.WaitForOrchestrationCompletion(ctx, id3)
	require.NoError(t, err)

	require.Equal(t, int32(300), f.calledCount.Load())
}
