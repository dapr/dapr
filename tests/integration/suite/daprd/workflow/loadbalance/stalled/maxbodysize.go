...
func (m *maxbodysize) Run(t *testing.T, ctx context.Context) {
	...
	newWorkers := func(t *testing.T, ctx context.Context) []*client.TaskHubGrpcClient {
		...
		for i := range clients {
			...
			require.NoError(t, registry.AddOrchestratorN("growing-history", func(wctx *task.OrchestrationContext) (any, error) {
	...
	id, err := clients[0].ScheduleNewOrchestration(ctx, "growing-history")
