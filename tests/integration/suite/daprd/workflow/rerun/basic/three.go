...
func (o *three) Setup(t *testing.T) []framework.Option {
...
func (o *three) Run(t *testing.T, ctx context.Context) {
	...
	newID, err := client.RerunWorkflowFromEvent(ctx, api.InstanceID("abc"), 2)
	require.NoError(t, err)
	assert.NotEqual(t, id, newID)

	_, err = client.WaitForOrchestrationCompletion(ctx, newID)
	require.NoError(t, err)
	assert.Equal(t, int64(3), act.Load())
}
