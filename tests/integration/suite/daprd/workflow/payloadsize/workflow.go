...
func (w *workflowoversize) Run(t *testing.T, ctx context.Context) {
	...
	require.NoError(t, w.workflow.Registry().AddOrchestratorN("growing-history", func(wctx *task.OrchestrationContext) (any, error) {
		for range 8 {
			var out string
	...
	require.NoError(t, w.workflow.Registry().AddActivityN("emit-chunk", func(task.ActivityContext) (any, error) {
		called.Add(1)
		return chunk, nil
	}))

	client := w.workflow.BackendClient(t, ctx)
	id, err := client.ScheduleNewOrchestration(ctx, "growing-history")
