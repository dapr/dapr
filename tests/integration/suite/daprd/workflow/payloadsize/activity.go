...
func (a *activityoversize) Run(t *testing.T, ctx context.Context) {
	...
	require.NoError(t, a.workflow.Registry().AddOrchestratorN("calls-large-activity", func(wctx *task.OrchestrationContext) (any, error) {
	...
	require.NoError(t, a.workflow.Registry().AddActivityN("echo", func(actx task.ActivityContext) (any, error) {
		called.Add(1)
		var in string
	...
	id, err := client.ScheduleNewOrchestration(ctx, "calls-large-activity")
