...
func init() {
	suite.Register(new(manually))
}

type manually struct {
	workflow *workflow.Workflow
}

func (m *manually) Setup(t *testing.T) []framework.Option {
...
func (m *manually) Run(t *testing.T, ctx context.Context) {
	...
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assert.Empty(c, m.workflow.Scheduler().ListAllKeys(t, ctx, "dapr/jobs"))
	}, time.Second*10, time.Millisecond*10)
