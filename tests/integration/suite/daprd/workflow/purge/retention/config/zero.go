...
func (z *zero) Setup(t *testing.T) []framework.Option {
...
func (z *zero) Run(t *testing.T, ctx context.Context) {
	...
	db := z.workflow.DB().GetConnection(t)
	tableName := z.workflow.DB().TableName()

	var count int
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		require.NoError(t, db.QueryRowContext(ctx, "SELECT COUNT(*) FROM "+tableName).Scan(&count))
		assert.Equal(c, 0, count)
		assert.Empty(c, z.workflow.Scheduler().ListAllKeys(t, ctx, "dapr/jobs"))
