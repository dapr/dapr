...
func (m *multi) Run(t *testing.T, ctx context.Context) {
	...
	stream1, err := client.SubscribeTopicEvents(ctx)
	require.NoError(t, err)
	require.NoError(t, stream1.Send(&rtv1.SubscribeTopicEventsRequest{
	...
	stream2, err := client.SubscribeTopicEvents(ctx)
	require.NoError(t, err)
	require.NoError(t, stream2.Send(&rtv1.SubscribeTopicEventsRequest{
		SubscribeTopicEventsRequestType: &rtv1.SubscribeTopicEventsRequest_InitialRequest{
			InitialRequest: &rtv1.SubscribeTopicEventsInitialRequest{
	...
	stream3, err := client.SubscribeTopicEvents(ctx)
	require.NoError(t, err)
	require.NoError(t, stream3.Send(&rtv1.SubscribeTopicEventsRequest{
		SubscribeTopicEventsRequestType: &rtv1.SubscribeTopicEventsRequest_InitialRequest{
			InitialRequest: &rtv1.SubscribeTopicEventsInitialRequest{
	...
	for stream, topic := range map[rtv1.Dapr_SubscribeTopicEventsClient]string{
		stream1: "a",
		stream2: "b",
		stream3: "c",
	} {
