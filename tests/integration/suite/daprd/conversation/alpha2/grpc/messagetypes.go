...
func (m *messagetypes) Run(t *testing.T, ctx context.Context) {
	...
	t.Run("multiple messages in conversation", func(t *testing.T) {
		...
		require.NotNil(t, resp.GetOutputs()[0].GetUsage())
		require.Equal(t, uint64(11), resp.GetOutputs()[0].GetUsage().GetCompletionTokens())
		require.Equal(t, uint64(11), resp.GetOutputs()[0].GetUsage().GetPromptTokens())
		require.Equal(t, uint64(22), resp.GetOutputs()[0].GetUsage().GetTotalTokens())
	})
}
