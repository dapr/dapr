...
func (c *customerrors) Run(t *testing.T, ctx context.Context) {
	...
	assert.EventuallyWithT(t, func(collect *assert.CollectT) {
		assert.True(collect, c.daprd.Metrics(collect, ctx).MatchMetricAndSum(2, "dapr_error_code_total", "category:state", "error_code:ERR_STATE_SAVE"))
		assert.True(collect, c.daprd.Metrics(collect, ctx).MatchMetricAndSum(1, "dapr_error_code_total", "category:state", "error_code:ERR_STATE_GET"))
		assert.True(collect, c.daprd.Metrics(collect, ctx).MatchMetricAndSum(1, "dapr_error_code_total", "category:state", "error_code:ERR_STATE_DELETE"))
