...
// resiliencynoretry tests that a resiliency policy with retries configured
// is correctly ignored for streaming (chunked-transfer) requests, while
// still being applied for normal fixed-length requests.
...
func (c *resiliencynoretry) Run(t *testing.T, ctx context.Context) {
	...
	t.Run("streaming request skips resiliency retries", func(t *testing.T) {
		// Streaming request (chunked, unknown Content-Length) should NOT be
		// retried even though a resiliency policy with retries is configured.
		pr, pw := io.Pipe()
		go func() {
	...
	t.Run("fixed-length request uses resiliency retries", func(t *testing.T) {
		// Normal request (known Content-Length) should be retried per the
		// resiliency policy (1 initial + 3 retries = 4 total calls).
		c.endpointCalls.Store(0)
		url := fmt.Sprintf("http://%s/v1.0/invoke/%s/method/always-fail",
			c.daprdSender.HTTPAddress(), c.daprdReceiver.AppID())
