...
func (r *precision) Run(t *testing.T, ctx context.Context) {
	...
	assertDurationWithTolerance(t, eMap, "test1", 1*time.Second, 5*time.Millisecond)
	assertDurationWithTolerance(t, eMap, "test2", 400*time.Millisecond, 5*time.Millisecond)
}

func assertDurationWithTolerance(t *testing.T, values map[string][]*request, key string, expectedDiff time.Duration, tolerance time.Duration) {
	...
	for i, e := range values[key] {
		...
		assert.True(t,
			actualDiff >= expectedDiff-tolerance && actualDiff <= expectedDiff+tolerance,
			"[%v]Expected execution time difference to be %v ± %v, but got %v", i,
