...
func (m *metrics) Setup(t *testing.T) []framework.Option {
	...
	app := app.New(t,
		...
		app.WithHandlerFunc("/job/failure", func(w http.ResponseWriter, r *http.Request) {
			fmt.Println("failure endpoint called")
...
func (m *metrics) Run(t *testing.T, ctx context.Context) {
	...
	t.Run("success count", func(t *testing.T) {
		...
		assert.EventuallyWithT(t, func(c *assert.CollectT) {
			...
			assert.Equal(t, 1, int(metrics["dapr_component_job_successCount|app_id:my_app|component:success|namespace:|operation:job_execution_op|success:true"]))
	...
	t.Run("failure count", func(t *testing.T) {
		...
		assert.EventuallyWithT(t, func(c *assert.CollectT) {
			metrics := m.daprd.Metrics(t, ctx)
			assert.Equal(t, 1, int(metrics["dapr_component_job_failureCount|app_id:my_app|component:failure|namespace:|operation:job_execution_op"]))
		}, time.Second*3, time.Millisecond*10)
	})
}
