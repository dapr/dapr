...
func (w *workflowPayloadSize) Run(t *testing.T, ctx context.Context) {
	...
	require.NoError(t, reg.AddActivityN("echo", func(actx task.ActivityContext) (any, error) {
		var in string
	...
	require.NoError(t, reg.AddOrchestratorN("payload_size_wf", func(wctx *task.OrchestrationContext) (any, error) {
	...
	id, err := client.ScheduleNewOrchestration(ctx, "payload_size_wf")
	require.NoError(t, err)
	meta, err := client.WaitForOrchestrationCompletion(ctx, id, api.WithFetchPayloads(true))
	require.NoError(t, err)
	require.True(t, api.OrchestrationMetadataIsComplete(meta))
