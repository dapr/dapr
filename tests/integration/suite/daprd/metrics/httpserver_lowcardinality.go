...
func init() {
	suite.Register(new(httpServerLowCardinality))
}

// httpServerLowCardinality tests daprd metrics for the HTTP server configured with low cardinality
type httpServerLowCardinality struct {
	base
}

func (m *httpServerLowCardinality) Setup(t *testing.T) []framework.Option {
...
func (m *httpServerLowCardinality) Run(t *testing.T, ctx context.Context) {
	m.beforeRun(t, ctx)
