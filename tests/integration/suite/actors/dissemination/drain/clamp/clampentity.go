...
func init() {
	suite.Register(new(clampentity))
}

type clampentity struct {
	app1  *actors.Actors
	app2  *actors.Actors
...
func (c *clampentity) Setup(t *testing.T) []framework.Option {
	...
	c.ll = logline.New(t, logline.WithStdoutLineContains(
		"drainOngoingCallTimeout (1m0s) for entities=abc meets or exceeds the dissemination timeout",
	))
...
func (c *clampentity) Run(t *testing.T, ctx context.Context) {
	...
	assert.Less(t, elapsed, time.Second*8,
		"clamped per-entity drain should fire before the dissemination timeout")
	assert.Greater(t, elapsed, time.Second,
		"clamped per-entity drain should still allow some grace period (~3s)")
