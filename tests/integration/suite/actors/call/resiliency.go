...
func (h *resiliency) Setup(t *testing.T) []framework.Option {
	...
	handler.HandleFunc("/actors/restest/", func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		// writing the body. This creates a window between headers being flushed
		// (and InvokeMethod returning) and the body data being available on the
		// HTTP stream. If the resiliency policy runner cancels the
		// timeout context during this window, the goroutine reading from
		// the HTTP body will fail without the fix.
		w.WriteHeader(nethttp.StatusOK)
...
func (h *resiliency) Run(t *testing.T, ctx context.Context) {
	...
	invoke := func(t *testing.T, address, method, expectedBody string) {
		...
		url := fmt.Sprintf("http://%s/v1.0/actors/restest/myactor/method/%s",
			address, method)
		...
		case <-h.headersSent:
		case <-ctx.Done():
			require.FailNow(t, "context cancelled waiting for headers")
		}

		// Signal the handler to write the body. With the bug, the
		// HTTP stream is already reset by now and the body data
		...
		case res = <-resCh:
		case <-ctx.Done():
			require.FailNow(t, "context cancelled waiting for response")
		}
		...
		assert.Equal(t, expectedBody, string(body),
			"response body mismatch (http+resiliency actor invocation regression)")
	}
