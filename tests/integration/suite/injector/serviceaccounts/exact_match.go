...
func init() {
	suite.Register(new(exactMatch))
}

// exactMatch verifies that an exact "namespace:name" entry in
// ALLOWED_SERVICE_ACCOUNTS authorizes the matching service account.
type exactMatch struct {
	injector *procinjector.Injector
}

func (e *exactMatch) Setup(t *testing.T) []framework.Option {
...
func (e *exactMatch) Run(t *testing.T, ctx context.Context) {
	...
	review := admissionv1.AdmissionReview{
		...
		Request: &admissionv1.AdmissionRequest{
			...
			UserInfo: authenticationv1.UserInfo{
				Username: "system:serviceaccount:custom-ns:custom-sa",
			},
	...
	assert.NotEmpty(t, ar.Response.Patch, "exact match should inject sidecar")
}
