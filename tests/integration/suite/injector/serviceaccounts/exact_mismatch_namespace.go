...
func init() {
	suite.Register(new(exactMismatchNamespace))
}

// exactMismatchNamespace verifies that a service account whose namespace does
// not match the configured entry is denied sidecar injection.
type exactMismatchNamespace struct {
	injector *procinjector.Injector
}

func (e *exactMismatchNamespace) Setup(t *testing.T) []framework.Option {
...
func (e *exactMismatchNamespace) Run(t *testing.T, ctx context.Context) {
	...
	review := admissionv1.AdmissionReview{
		...
		Request: &admissionv1.AdmissionRequest{
			...
			UserInfo: authenticationv1.UserInfo{
				Username: "system:serviceaccount:other-ns:custom-sa",
			},
	...
	assert.Empty(t, ar.Response.Patch, "wrong namespace should not inject sidecar")
}
