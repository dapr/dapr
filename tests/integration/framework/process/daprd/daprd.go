...
// API), then calls Cleanup so the framework can assert the exit code.
// kill.Interrupt is a no-op on an already-dead process. The HTTP port is
// polled; once dialing fails, the process has exited.
