...
func (p *Permutation) Execute(t *testing.T, ctx context.Context) {
	...
	newWorkers := func(t *testing.T, ctx context.Context, version string, idxs []int) {
		...
		for _, idx := range idxs {
			...
			require.NoError(t, registry.AddVersionedOrchestratorN("workflow", version, true, func(ctx *task.OrchestrationContext) (any, error) {
	...
	id, err := observers[0].ScheduleNewOrchestration(ctx, "workflow")
	require.NoError(t, err)
	wf.WaitForOrchestratorStartedEvent(t, ctx, observers[0], id)
