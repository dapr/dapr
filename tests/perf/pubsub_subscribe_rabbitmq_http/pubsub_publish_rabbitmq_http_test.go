...
package pubsub_subscribe_rabbitmq_http
...
const (
	...
	testAppName                     = "testapp"
	testLabel                       = "pubsub_subscribe_rabbitmq_http"
	defaultSubscribeHttpThresholdMs = 500
)
	...
	testApps := []kube.AppDescription{
		{
			AppName:           testAppName,
			DaprEnabled:       true,
			ImageName:         "perf-pubsub_subscribe_rabbitmq_http",
			Replicas:          1,
			IngressEnabled:    true,
			MetricsEnabled:    true,
			AppPort:           3000,
...
func runTest(t *testing.T, testAppURL, httpReqDurationThresholdMs string) {
	t.Logf("Starting test with RabbitMQ subscriber")
...
func TestPubsubSubscribeRabbitMQHttpPerformance(t *testing.T) {
	...
	if threshold == "" {
		threshold = strconv.Itoa(defaultSubscribeHttpThresholdMs)
	}
	runTest(t, testAppURL, threshold)
}
