...
func TestMain(m *testing.M) {
	utils.SetupLogs("pubsub_actor_test")

	testApps := []kube.AppDescription{
		{
			AppName:           serviceApplicationName,
			DaprEnabled:       true,
			ImageName:         "perf-actorpubsub",
			Replicas:          1,
			IngressEnabled:    true,
			AppPort:           3000,
			DaprCPULimit:      "4.0",
	...
	tr = runner.NewTestRunner("actorpubsubperftest", testApps, nil, nil)
	os.Exit(tr.Start(m))
}

func TestPubsubPublishActorsPerformance(t *testing.T) {
	p := perf.Params()
	...
	// Perform dapr test
	endpoint := fmt.Sprintf("http://127.0.0.1:3500/v1.0-alpha1/actors/%s/{uuid}/publish/%s/%s", actorType, defaultpubsubName, defaultTopicNameHTTP)
