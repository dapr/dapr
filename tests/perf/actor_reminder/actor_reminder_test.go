...
func TestActorReminderRegistrationPerformance(t *testing.T) {
	...
	t.Logf("Actual QPS: %.2f, expected QPS: %.0f", daprResult.ActualQPS, targetQPS)
...
func TestActorReminderTriggerPerformance(t *testing.T) {
	...
	worker := func(i int) {
		assert.EventuallyWithT(t, func(c *assert.CollectT) {
			_, err = utils.HTTPPost(fmt.Sprintf("%s/actors/%s/%d/reminders/myreminder%d", testAppURL, actorType, i, i), reminderB)
	...
	require.GreaterOrEqualf(t, dueTime*time.Second, done, "expected to create reminders %d in less than %ss", reminderCount, dueTime)
	...
	// Allow tolerance for CI variance (low end observed ~9873)
	assert.InDelta(t, targetTriggerQPS, qps, 3200)
}
