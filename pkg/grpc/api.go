...
func (a *api) PerformTransaction(ctx context.Context, in *dapr_pb.MultiStateEnvelope) (*empty.Empty, error) {
	if a.stateStores == nil || len(a.stateStores) == 0 {
		return &empty.Empty{}, errors.New("ERR_STATE_STORE_NOT_CONFIGURED")
	}
	...
	for _, inputReq := range in.Requests {
		...
		case state.Upsert:
			setReq := state.SetRequest{
				...
				Metadata: inputReq.Request.Metadata,
				Value:    inputReq.Request.Value.Value,
				Options:  a.getSetStateOptions(inputReq.Request),
			}
		...
		case state.Delete:
			delReq := state.DeleteRequest{
				Key:      a.getModifiedStateKey(inputReq.Request.Key),
				Metadata: inputReq.Request.Metadata,
				Options:  a.getDeleteStateOption(inputReq.Request),
			}
			req := state.TransactionalRequest{
...
func (a *api) getSetStateOptions(r *dapr_pb.StateRequest) (o state.SetStateOption) {

	if r.Options != nil {
		o.Consistency = r.Options.Consistency
		o.Concurrency = r.Options.Concurrency

		if r.Options.RetryPolicy != nil {
			o.RetryPolicy = state.RetryPolicy{
				Threshold: int(r.Options.RetryPolicy.Threshold),
				Pattern:   r.Options.RetryPolicy.Pattern,
			}
...
func (a *api) getDeleteStateOption(r *dapr_pb.StateRequest) (o state.DeleteStateOption) {
