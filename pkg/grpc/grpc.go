...
func (g *Manager) CreateLocalChannel(port, maxConcurrency int, timeout time.Duration,  spec config.TracingSpec, sslEnabled bool, maxRequestBodySize int) (channel.AppChannel, error) {
	...
	g.AppClient = conn
	ch := grpc_channel.CreateLocalChannel(port, maxConcurrency, timeout,conn, spec, maxRequestBodySize)
	return ch, nil
}
