...
func (s *server) getMiddlewareOptions() []grpc_go.ServerOption {
	...
	unaryServerInterceptor := diag.SetTracingInGRPCMiddlewareUnary(s.tracingSpec)
	...
	opts = append(
		opts,
		grpc_go.StreamInterceptor(diag.SetTracingInGRPCMiddlewareStream(s.tracingSpec)),
