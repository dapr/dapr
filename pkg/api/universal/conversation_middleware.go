...
func NewStreamingPIIScrubber(enabled bool, windowSize int) (*StreamingPIIScrubber, error) {
...
// IsTokenBoundary determines if a chunk ends at a token boundary (heuristic)
func (s *StreamingPIIScrubber) IsTokenBoundary(chunk []byte) *bool {
	if len(chunk) == 0 {
		result := false
		return &result
	}
	...
	result := lastChar == ' ' || lastChar == '.' || lastChar == ',' ||
...
func NewStreamingPipeline(piiEnabled bool, logger interface{}) (*StreamingPIIScrubber, error) {
	return NewStreamingPIIScrubber(piiEnabled, 50) // 50 character look-ahead window
}
