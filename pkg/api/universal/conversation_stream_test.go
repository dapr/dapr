...
func TestConverseStreamAlpha1_ComponentValidation(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			inputs:        []*runtimev1pb.ConversationInput{{Message: "test"}},
...
func TestConverseStreamAlpha1_ComponentError(t *testing.T) {
	...
	err := api.ConverseStreamAlpha1(req, stream)
	require.NoError(t, err) // Error should be sent via stream, not returned
	...
	for _, msg := range stream.messages {
		if errMsg := msg.GetError(); errMsg != nil {
			hasError = true
			assert.Contains(t, errMsg.GetMessage(), "mock conversation error")
			assert.Equal(t, int32(500), errMsg.GetCode())
		}
	}
	assert.True(t, hasError, "Expected error message in stream")
}
...
func TestStreamingPIIScrubber(t *testing.T) {
	t.Run("PII scrubbing disabled", func(t *testing.T) {
		scrubber, err := NewStreamingPIIScrubber(false, 10)
		require.NoError(t, err)

		input := []byte("My SSN is 123-45-6789")
		result := scrubber.ProcessChunk(input)
		assert.Equal(t, input, result, "Should return input unchanged when disabled")
	})

	t.Run("PII scrubbing enabled with buffering", func(t *testing.T) {
		scrubber, err := NewStreamingPIIScrubber(true, 5)
		require.NoError(t, err)
	...
	t.Run("Token boundary detection", func(t *testing.T) {
		scrubber, err := NewStreamingPIIScrubber(false, 10)
		require.NoError(t, err)
		...
		tests := []struct {
			...
			{[]byte("hello "), true},    // space
			{[]byte("world."), true},    // period
			{[]byte("test,"), true},     // comma
			{[]byte("end!"), true},      // exclamation
			{[]byte("question?"), true}, // question mark
			{[]byte("line\n"), true},    // newline
			{[]byte("middle"), false},   // no boundary
			{[]byte(""), false},         // empty
		}

		for _, tt := range tests {
			result := scrubber.IsTokenBoundary(tt.input)
			require.NotNil(t, result)
			assert.Equal(t, tt.expected, *result, "Token boundary detection for %q", string(tt.input))
		}
	})
}
