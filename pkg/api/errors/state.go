...
func (s *StateStoreError) NotFound(appID string) error {
	...
	return s.build(
		errors.NewBuilder(
			...
			msg,
			messages.RecordAndGet(errorcodes.StateStoreNotFound),
		),
		errors.CodeNotFound,
		meta,
	)
...
func (s *StateStoreError) NotConfigured(appID string) error {
	...
	return s.build(
		errors.NewBuilder(
			...
			msg,
			messages.RecordAndGet(errorcodes.StateStoreNotConfigured),
		),
...
func (s *StateStoreError) InvalidKeyName(key string, msg string) error {
	return s.build(
		errors.NewBuilder(
			...
			http.StatusBadRequest,
			msg,
			messages.RecordAndGet(errorcodes.CommonMalformedRequest),
...
func (s *StateStoreError) TooManyTransactionalOps(count int, max int) error {
	return s.build(
		errors.NewBuilder(
			...
			messages.RecordAndGet(errorcodes.StateStoreTooManyTransactions),
		),
...
func (s *StateStoreError) QueryUnsupported() error {
	return s.build(
		errors.NewBuilder(
			...
			messages.RecordAndGet(errorcodes.StateStoreNotSupported),
		),
...
func (s *StateStoreError) QueryFailed(detail string) error {
	return s.build(
		errors.NewBuilder(
			...
			messages.RecordAndGet(errorcodes.StateQuery),
		),
