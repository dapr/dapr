...
func (s *serviceMetrics) Init(appID string) error {
	...
	return view.Register(
		utils.NewMeasureView(s.componentLoaded, []tag.Key{appIDKey}, view.Count()),
		utils.NewMeasureView(s.componentInitCompleted, []tag.Key{appIDKey, componentKey}, view.Count()),
		utils.NewMeasureView(s.componentInitFailed, []tag.Key{appIDKey, componentKey, failReasonKey}, view.Count()),

		utils.NewMeasureView(s.mtlsInitCompleted, []tag.Key{appIDKey}, view.Count()),
		utils.NewMeasureView(s.mtlsInitFailed, []tag.Key{appIDKey, failReasonKey}, view.Count()),
		utils.NewMeasureView(s.mtlsWorkloadCertRotated, []tag.Key{appIDKey}, view.Count()),
		utils.NewMeasureView(s.mtlsWorkloadCertRotatedFailed, []tag.Key{appIDKey, failReasonKey}, view.Count()),

		utils.NewMeasureView(s.actorStatusReportTotal, []tag.Key{appIDKey, actorTypeKey, actorIDKey, operationKey}, view.Count()),
		utils.NewMeasureView(s.actorStatusReportFailedTotal, []tag.Key{appIDKey, actorTypeKey, actorIDKey, operationKey, failReasonKey}, view.Count()),
		utils.NewMeasureView(s.actorTableOperationRecvTotal, []tag.Key{appIDKey, actorTypeKey, actorIDKey, operationKey}, view.Count()),
		utils.NewMeasureView(s.actorRebalancedTotal, []tag.Key{appIDKey, actorTypeKey, actorIDKey}, view.Count()),
		utils.NewMeasureView(s.actorActivatedTotal, []tag.Key{appIDKey, actorTypeKey, actorIDKey}, view.Count()),
		utils.NewMeasureView(s.actorActivatedFailedTotal, []tag.Key{appIDKey, actorTypeKey, actorIDKey}, view.Count()),
		utils.NewMeasureView(s.actorDeactivationTotal, []tag.Key{appIDKey, actorTypeKey, actorIDKey}, view.Count()),
		utils.NewMeasureView(s.actorDeactivationFailedTotal, []tag.Key{appIDKey, actorTypeKey, actorIDKey}, view.Count()),
	)
}

// ComponentLoaded records metric when component is loaded successfully
func (s *serviceMetrics) ComponentLoaded() {
	stats.RecordWithTags(s.ctx, utils.WithTags(appIDKey, s.appID), s.componentLoaded.M(1))
}
...
func (s *serviceMetrics) ComponentInitialized(component string) {
	stats.RecordWithTags(
		s.ctx,
		utils.WithTags(appIDKey, s.appID, componentKey, component),
...
func (s *serviceMetrics) ComponentInitFailed(component string, reason string) {
	stats.RecordWithTags(
		s.ctx,
		utils.WithTags(appIDKey, s.appID, componentKey, component, failReasonKey, reason),
...
func (s *serviceMetrics) MTLSInitCompleted() {
	stats.RecordWithTags(s.ctx, utils.WithTags(appIDKey, s.appID), s.mtlsInitCompleted.M(1))
}

// MTLSInitFailed records metric when component initialization is failed
func (s *serviceMetrics) MTLSInitFailed(reason string) {
	stats.RecordWithTags(
		s.ctx, utils.WithTags(appIDKey, s.appID, failReasonKey, reason),
...
func (s *serviceMetrics) MTLSWorkLoadCertRotationCompleted() {
	stats.RecordWithTags(s.ctx, utils.WithTags(appIDKey, s.appID), s.mtlsWorkloadCertRotated.M(1))
}
...
func (s *serviceMetrics) ActorStatusReported(operation string) {
	stats.RecordWithTags(
		s.ctx, utils.WithTags(appIDKey, s.appID, operationKey, operation),
...
func (s *serviceMetrics) ActorStatusReportFailed(operation string, reason string) {
	stats.RecordWithTags(
		s.ctx, utils.WithTags(appIDKey, s.appID, operationKey, operation, failReasonKey, reason),
...
func (s *serviceMetrics) ActorRebalanced(actorType, actorID string) {
	stats.RecordWithTags(
		s.ctx,
		utils.WithTags(appIDKey, s.appID, actorTypeKey, actorType, actorIDKey, actorID),
...
func (s *serviceMetrics) ActorActivationFailed(actorType, actorID string, reason string) {
	stats.RecordWithTags(
		s.ctx,
		utils.WithTags(appIDKey, s.appID, actorTypeKey, actorType, actorIDKey, actorID, failReasonKey, reason),
