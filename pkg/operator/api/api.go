...
import (
	...
	"github.com/dapr/dapr/pkg/operator/api/syncer"
...
type apiServer struct {
	...
	compSyncer *syncer.Syncer[componentsapi.Component]
...
func NewAPIServer(opts Options) Server {
	return &apiServer{
		...
		compSyncer: syncer.New[componentsapi.Component](syncer.Options{
...
func (a *apiServer) ComponentUpdate(in *operatorv1pb.ComponentUpdateRequest, srv operatorv1pb.Operator_ComponentUpdateServer) error { //nolint:nosnakecase
	...
	ch, err := a.compSyncer.WatchUpdates(srv.Context(), in.GetNamespace())
	if err != nil {
		return err
	}
