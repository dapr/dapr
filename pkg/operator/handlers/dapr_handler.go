...
func (h *DaprHandler) patchDaprService(ctx context.Context, expectedService types.NamespacedName, wrapper ObjectWrapper, daprSvc corev1.Service) error {
	appID := h.getAppID(wrapper)
	service := h.createDaprServiceValues(expectedService, wrapper, appID)
...
func (h *DaprHandler) createDaprServiceValues(expectedService types.NamespacedName, wrapper ObjectWrapper, appID string) *corev1.Service {
