...
// It first attempts to get a free port that is between start and start+32767, which is calculated in a deterministic way based on the current process' environment.
...
func GetStablePort(start int, appID string) (int, error) {
	...
	// Get the first 15 bits (0-32767) as "random number"
	rnd := (int(h[0]) | int(h[1])<<8) & 0x7FFF
	port := start + rnd
