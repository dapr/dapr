...
type ServiceInvocationClient interface {
	...
	// The caller and callee must send at least one message in the stream. If only 1 message is sent in each direction, that message must contain both a `request`/`response` and a `payload` with `complete=true` (the `payload` may be empty).
