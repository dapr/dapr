...
type ServiceInvocationClient interface {
	...
	// Although this uses a bi-directional stream, it behaves as a "simple RPC" in which the caller sends the full request (chunked in multiple messages in the stream), then reads the full response (chunked using the stream).
	...
	// - The last message in the stream MUST contain a `payload` with `complete=true``. That message is assumed to be the last one from the sender and no more messages are to be sent in the stream in the same direction after that.
	// The caller and callee must send at least one message in the stream. If only 1 message is sent in each direction, that message must contain both a `request`/`response` and a `payload` with `complete=true` (the `payload` may not contain any data).
