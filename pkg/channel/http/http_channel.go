...
func (h *Channel) HealthProbe(ctx context.Context) (bool, error) {
	...
	if err != nil {
		...
		diag.DefaultHTTPMonitoring.AppHealthProbeCompleted(ctx, strconv.Itoa(http.StatusInternalServerError), startRequest)
		//nolint:nilerr
		return false, nil
	}
	...
	diag.DefaultHTTPMonitoring.AppHealthProbeCompleted(ctx, strconv.Itoa(channelResp.StatusCode), startRequest)
