...
func TestRegistry(t *testing.T) {
	...
	t.Run("middleware is registered", func(t *testing.T) {
		...
		testRegistry.RegisterComponent(func(_ logger.Logger) http.FactoryMethod {
			return func(context.Context, h.Metadata) (httpMiddleware.Middleware, error) {
				return mock, nil
			}
		}, middlewareName)
		testRegistry.RegisterComponent(func(_ logger.Logger) http.FactoryMethod {
		...
		// assert v0 and v1
		p, e := testRegistry.Create(context.Background(), componentName, "v0", metadata, "")
		assert.NoError(t, e)
		assert.True(t, reflect.ValueOf(mock) == reflect.ValueOf(p))
		p, e = testRegistry.Create(context.Background(), componentName, "v1", metadata, "")
		assert.NoError(t, e)
		assert.True(t, reflect.ValueOf(mock) == reflect.ValueOf(p))

		// assert v2
		pV2, e := testRegistry.Create(context.Background(), componentName, "v2", metadata, "")
		assert.NoError(t, e)
		assert.True(t, reflect.ValueOf(mockV2) == reflect.ValueOf(pV2))

		// check case-insensitivity
		pV2, e = testRegistry.Create(context.Background(), strings.ToUpper(componentName), "V2", metadata, "")
		assert.NoError(t, e)
		assert.True(t, reflect.ValueOf(mockV2) == reflect.ValueOf(pV2))
	})
	...
	t.Run("middleware is not registered", func(t *testing.T) {
		...
		// act
		p, actualError := testRegistry.Create(context.Background(), componentName, "v1", metadata, "")
