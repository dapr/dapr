...
func (i *injector) Run(ctx context.Context, tlsConfig *tls.Config, sentryID spiffeid.ID, currentTrustAnchors currentTrustAnchorsFn) error {
	...
	for {
		...
		for n := range *sched.Spec.Replicas { // nolint: typecheck // go 1.22 allows this but golangci-lint does not as it is experimental https://github.com/golangci/golangci-lint/issues/4627
