...
func mTLSEnabled(daprClient scheme.Interface) bool {
	...
	if err != nil {
		log.Errorf("Fail to load dapr configuration from k8s, use default value %t for mTLSEnabled: %s", defaultMtlsEnabled, err)
	...
	log.Infof("Dapr system configuration is not found, use default value %t for mTLSEnabled", defaultMtlsEnabled)
	return defaultMtlsEnabled
}
