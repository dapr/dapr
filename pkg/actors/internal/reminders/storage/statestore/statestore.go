...
func (r *Statestore) startReminder(reminder *api.Reminder, stop *reminderStop) error {
	...
	log.Debugf("[startReminder] Got reminder track for key: %s, track: %+v", reminderKey, track)
	...
	loop:
		for {
			...
			if errors.Is(err, actorerrors.ErrReminderCanceled) {
				log.Infof("[startReminder] Reminder %s was canceled", reminderKey)
				nextTimer = nil
				break loop
			}
			...
			if !active {
				log.Infof("[startReminder] Reminder %s expired, dueTime: %s, period: %s", reminderKey, reminder.DueTime, reminder.Period)
				nextTimer = nil
				break loop
			}
