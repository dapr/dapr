...
func TestIsChildAlreadyResolved(t *testing.T) {
	t.Parallel()

	childStart := func(id int32) *backend.HistoryEvent {
		return &backend.HistoryEvent{
			EventId:   -1,
			Timestamp: timestamppb.Now(),
			EventType: &protos.HistoryEvent_ExecutionStarted{ExecutionStarted: &protos.ExecutionStartedEvent{
				Name:           "child",
				ParentInstance: &protos.ParentInstanceInfo{TaskScheduledId: id},
			}},
		}
	}
	detachedStart := &backend.HistoryEvent{
		EventId:   -1,
		Timestamp: timestamppb.Now(),
		EventType: &protos.HistoryEvent_ExecutionStarted{ExecutionStarted: &protos.ExecutionStartedEvent{Name: "detached"}},
	}
	childFailed := &backend.HistoryEvent{
		EventId:   -1,
		Timestamp: timestamppb.Now(),
		EventType: &protos.HistoryEvent_ChildWorkflowInstanceFailed{ChildWorkflowInstanceFailed: &protos.ChildWorkflowInstanceFailedEvent{TaskScheduledId: 2}},
	}

	tests := []struct {
		name    string
		start   *backend.HistoryEvent
		history []*backend.HistoryEvent
		inbox   []*backend.HistoryEvent
		want    bool
	}{
		{name: "nothing recorded", start: childStart(1), want: false},
		{name: "completion in history", start: childStart(1), history: []*backend.HistoryEvent{childCompleted(1)}, want: true},
		{name: "failure in history is also a resolution", start: childStart(2), history: []*backend.HistoryEvent{childFailed}, want: true},
		{name: "resolution in inbox still counts", start: childStart(1), inbox: []*backend.HistoryEvent{childCompleted(1)}, want: true},
		{name: "resolution exists for a different id", start: childStart(1), history: []*backend.HistoryEvent{childCompleted(9)}, want: false},
		{name: "a task resolution does not resolve a child", start: childStart(1), history: []*backend.HistoryEvent{taskCompleted(1)}, want: false},
		{name: "a detached spawn is never resolved", start: detachedStart, history: []*backend.HistoryEvent{childCompleted(0)}, want: false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, tt.want, dedup.IsChildAlreadyResolved(tt.start, tt.history, tt.inbox))
		})
	}
}
