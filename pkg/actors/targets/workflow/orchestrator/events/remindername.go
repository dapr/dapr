...
func EventReminderName(prefix string, e *backend.HistoryEvent) string {
	...
	case *protos.HistoryEvent_SubOrchestrationInstanceCompleted:
		return fmt.Sprintf("%s-cwc-%d", prefix, evt.SubOrchestrationInstanceCompleted.GetTaskScheduledId())
	case *protos.HistoryEvent_SubOrchestrationInstanceFailed:
		return fmt.Sprintf("%s-cwf-%d", prefix, evt.SubOrchestrationInstanceFailed.GetTaskScheduledId())
