...
func (o *orchestrator) saveInternalState(ctx context.Context, state *wfenginestate.State) error {
	...
	if err = o.actorState.TransactionalStateOperation(ctx, true, req, false); err != nil {
		...
		// peer's view; our in-memory state is stale. Invalidate the cache so
		// the next operation reloads, and log distinctly so the line
		// distinguishes peer-write conflicts from real save failures. The
