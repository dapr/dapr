...
func TestMonitorLeadership(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		idx := i % numServers
		i++
		if placementServers[idx].raftNode.IsLeader() && placementServers[idx].hasLeadership.Load() {
			leaderIdx = idx
			return true
		}
		return false
	...
	underlyingRaftServers[leaderIdx].LeadershipTransferToServer(hashicorpRaft.ServerID(placementServers[secondServerID].raftNode.GetID()), hashicorpRaft.ServerAddress(placementServers[secondServerID].raftNode.GetRaftBind()))
	require.Eventually(t, func() bool {
		return placementServers[secondServerID].raftNode.IsLeader() && placementServers[secondServerID].hasLeadership.Load()
	...
	require.Eventually(t, func() bool {
		future = underlyingRaftServers[secondServerID].LeadershipTransferToServer(hashicorpRaft.ServerID(placementServers[leaderIdx].raftNode.GetID()), hashicorpRaft.ServerAddress(placementServers[leaderIdx].raftNode.GetRaftBind()))
		return future.Error() == nil
	}, time.Second*15, 500*time.Millisecond, "raft leadership transfer timeout")

	require.Eventually(t, func() bool {
		return placementServers[leaderIdx].raftNode.IsLeader() && placementServers[leaderIdx].hasLeadership.Load()
