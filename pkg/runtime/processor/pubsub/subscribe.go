...
func (p *pubsub) StartSubscriptions(ctx context.Context) error {
	...
	if err := p.StopSubscriptions(); err != nil {
		return err
	}
...
func (p *pubsub) StopSubscriptions() error {
	p.lock.Lock()
	defer p.lock.Unlock()

	var errs []error
	for pubsubName, topics := range p.compStore.GetTopicRoutes() {
		for topic := range topics {
			if err := p.unsubscribeTopic(pubsubName, topic); err != nil {
				errs = append(errs, err)
				continue
			}

			p.compStore.DeleteTopicRoute(topicKey(pubsubName, topic))
		}
	}
