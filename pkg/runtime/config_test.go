...
func Test_toInternal(t *testing.T) {
	...
	assert.Equal(t, "http", string(intc.applicationProtocol))
	...
	assert.Equal(t, 8080, intc.applicationPort)
	assert.Equal(t, 7070, intc.profilePort)
	assert.Equal(t, true, intc.enableProfiling)
	assert.Equal(t, 1, intc.maxConcurrency)
	...
	assert.Equal(t, "1.1.1.1", intc.appChannelAddress)
}
