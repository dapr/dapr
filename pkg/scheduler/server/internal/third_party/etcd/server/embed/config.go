...
// grabbed https://raw.githubusercontent.com/etcd-io/etcd/refs/tags/v3.5.16/server/embed/config.go
...
func (cfg *Config) Validate() error {
	...
	if err := checkBindURLs(cfg.ListenPeerUrls, cfg.PeerTLSInfo.Security.MTLSEnabled()); err != nil {
		return err
	}
	if err := checkBindURLs(cfg.ListenClientUrls, cfg.ClientTLSInfo.Security.MTLSEnabled()); err != nil {
		return err
	}
	if err := checkBindURLs(cfg.ListenClientHttpUrls, cfg.ClientTLSInfo.Security.MTLSEnabled()); err != nil {
		return err
	}
	...
	if err := checkBindURLs(cfg.ListenMetricsUrls, cfg.ClientTLSInfo.Security.MTLSEnabled()); err != nil {
		return err
	}
	if err := checkHostURLs(cfg.AdvertisePeerUrls, cfg.PeerTLSInfo.Security.MTLSEnabled()); err != nil {
		addrs := cfg.getAdvertisePeerUrls()
		return fmt.Errorf(`--initial-advertise-peer-urls %q must be "host:port" (%v)`, strings.Join(addrs, ","), err)
	}
	if err := checkHostURLs(cfg.AdvertiseClientUrls, cfg.ClientTLSInfo.Security.MTLSEnabled()); err != nil {
...
func (cfg *Config) PeerURLsMapAndToken(which string) (urlsmap types.URLsMap, token string, err error) {
	...
	case cfg.DNSCluster != "":
		...
		if strings.Contains(clusterStr, "https://") && cfg.PeerTLSInfo.Security != nil {
			cfg.ServerName = cfg.DNSCluster
		}
...
func checkBindURLs(urls []url.URL, secure bool) error {
	for _, url := range urls {
		if url.Scheme == "unix" || url.Scheme == "unixs" {
			continue
		}
		if secure && url.Scheme == "http" {
			return errors.New("bind url scheme should be secure")
		}
...
func checkHostURLs(urls []url.URL, secure bool) error {
	for _, url := range urls {
		if secure && url.Scheme == "http" {
			return errors.New("host url scheme should be secure")
		}
		host, _, err := net.SplitHostPort(url.Host)
		if err != nil {
			return err
