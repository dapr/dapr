...
import (
	...
	osrafthttp "go.etcd.io/etcd/server/v3/etcdserver/api/rafthttp"
...
func NewListener(u url.URL, tlsinfo *transport.TLSInfo) (net.Listener, error) {
	return transport.NewListenerWithOpts(u.Host, u.Scheme, transport.WithTLSInfo(tlsinfo), transport.WithTimeout(osrafthttp.ConnReadTimeout, osrafthttp.ConnWriteTimeout))
}
...
func newStreamRoundTripper(tlsInfo transport.TLSInfo, dialTimeout time.Duration) (http.RoundTripper, error) {
	return transport.NewTimeoutTransport(tlsInfo, dialTimeout, osrafthttp.ConnReadTimeout, osrafthttp.ConnWriteTimeout)
}
...
func checkPostResponse(lg *zap.Logger, resp *http.Response, body []byte, req *http.Request, to types.ID) error {
	...
	case http.StatusPreconditionFailed:
		...
		case osrafthttp.ErrClusterIDMismatch.Error():
			if lg != nil {
				lg.Error(
			...
			return osrafthttp.ErrClusterIDMismatch
		default:
