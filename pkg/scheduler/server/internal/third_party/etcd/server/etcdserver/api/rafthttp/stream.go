	...
	for {
		...
		case <-heartbeatc:
			...
			if err == nil {
				flusher.Flush()
				batched = 0
				rafthttpSentBytes.WithLabelValues(cw.peerID.String()).Add(float64(unflushed))
				unflushed = 0
				continue
			}
...
func (cr *streamReader) decodeLoop(rc io.ReadCloser, t streamType) error {
	...
	for {
		...
		rafthttpReceivedBytes.WithLabelValues(types.ID(m.From).String()).Add(float64(m.Size()))
