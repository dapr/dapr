...
func promoteMemberHTTP(ctx context.Context, url string, id uint64, peerRt http.RoundTripper) ([]*membership.Member, error) {
	...
	if resp.StatusCode == http.StatusPreconditionFailed {
		...
		if strings.Contains(string(b), etcdserver.ErrLearnerNotReady.Error()) {
			return nil, etcdserver.ErrLearnerNotReady
		}
...
func convertToClusterVersion(v string) (*semver.Version, error) {
	...
	if err != nil {
		...
		if err != nil {
			return nil, etcdserver.ErrWrongDowngradeVersionFormat
		}
	}
