...
func WriteError(lg *zap.Logger, w http.ResponseWriter, r *http.Request, err error) {
	...
	default:
		switch err {
		case osetcdserver.ErrTimeoutDueToLeaderFail, osetcdserver.ErrTimeoutDueToConnectionLost, osetcdserver.ErrNotEnoughStartedMembers,
			osetcdserver.ErrUnhealthy:
			if lg != nil {
				lg.Warn(
					"v2 response error",
					zap.String("remote-addr", r.RemoteAddr),
