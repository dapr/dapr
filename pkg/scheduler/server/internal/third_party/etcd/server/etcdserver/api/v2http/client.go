...
import (
	...
	osetcdserver "go.etcd.io/etcd/server/v3/etcdserver"
	...
	"github.com/dapr/dapr/pkg/scheduler/server/internal/third_party/etcd/server/etcdserver/api/v2auth"
)
...
func writeKeyError(lg *zap.Logger, w http.ResponseWriter, err error) {
	...
	default:
		switch err {
		case osetcdserver.ErrTimeoutDueToLeaderFail, osetcdserver.ErrTimeoutDueToConnectionLost:
			if lg != nil {
				lg.Warn(
					"v2 response error",
