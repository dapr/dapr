...
func (s *EtcdServer) waitAppliedIndex() error {
	...
	case <-s.ApplyWait():
	case <-s.stopping:
		return etcdserver.ErrStopped
	case <-time.After(applyTimeout):
		return etcdserver.ErrTimeoutWaitAppliedIndex
	}
...
func (s *EtcdServer) LeaseRenew(ctx context.Context, id lease.LeaseID) (int64, error) {
	...
	if cctx.Err() == context.DeadlineExceeded {
		return -1, etcdserver.ErrTimeout
	}
	return -1, etcdserver.ErrCanceled
}
...
func (s *EtcdServer) leaseTimeToLive(ctx context.Context, r *pb.LeaseTimeToLiveRequest) (*pb.LeaseTimeToLiveResponse, error) {
	if s.isLeader() {
		...
		if le.Demoted() {
			...
			return nil, etcdserver.ErrLeaderChanged
		}
		return resp, nil
	}
...
func (s *EtcdServer) waitLeader(ctx context.Context) (*membership.Member, error) {
	...
	for leader == nil {
		...
		case <-ctx.Done():
			return nil, etcdserver.ErrNoLeader
		}
	}
...
func (s *EtcdServer) processInternalRaftRequestOnce(ctx context.Context, r pb.InternalRaftRequest) (*applyResult, error) {
	...
	if ci > ai+maxGapBetweenApplyAndCommitIndex {
		return nil, etcdserver.ErrTooManyRequests
	}
	...
	if len(data) > int(s.Cfg.MaxRequestBytes) {
		return nil, etcdserver.ErrRequestTooLarge
	}
...
func isStopped(err error) bool {
	return err == raft.ErrStopped || err == etcdserver.ErrStopped
}
...
func (s *EtcdServer) requestCurrentIndex(leaderChangedNotifier <-chan struct{}, requestId uint64) (uint64, error) {
	...
	for {
		...
		case <-leaderChangedNotifier:
			...
			return 0, etcdserver.ErrLeaderChanged
		...
		case <-errorTimer.C:
			...
			slowReadIndex.Inc()
			return 0, etcdserver.ErrTimeout
		case <-s.stopping:
			return 0, etcdserver.ErrStopped
		}
	}
}
...
func (s *EtcdServer) Downgrade(ctx context.Context, r *pb.DowngradeRequest) (*pb.DowngradeResponse, error) {
	...
	default:
		return nil, etcdserver.ErrUnknownMethod
	}
}
...
func (s *EtcdServer) downgradeValidate(ctx context.Context, v string) (*pb.DowngradeResponse, error) {
	...
	cv := s.ClusterVersion()
	if cv == nil {
		return nil, etcdserver.ErrClusterVersionUnavailable
	}
...
func (s *EtcdServer) downgradeCancel(ctx context.Context) (*pb.DowngradeResponse, error) {
	...
	downgradeInfo := s.cluster.DowngradeInfo()
	if !downgradeInfo.Enabled {
		return nil, etcdserver.ErrNoInflightDowngrade
	}
