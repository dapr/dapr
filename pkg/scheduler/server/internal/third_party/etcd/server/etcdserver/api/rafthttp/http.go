...
func (h *pipelineHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	rafthttpReceivedBytes.WithLabelValues(types.ID(m.From).String()).Add(float64(len(b)))
...
func (h *snapshotHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	msgSize := m.Size()
	rafthttpReceivedBytes.WithLabelValues(from).Add(float64(msgSize))
	...
	rafthttpReceivedBytes.WithLabelValues(from).Add(float64(n))
