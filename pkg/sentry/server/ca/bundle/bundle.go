...
type X509 struct {
	...
	// NextIssChainPEM is the PEM encoded pending issuer certificate chain
	// generated by CA renewal. The pending issuer is not used for signing until
	// the trust anchor propagation grace period has elapsed.
	NextIssChainPEM []byte
	// NextIssKeyPEM is the PEM encoded pending issuer private key.
	NextIssKeyPEM []byte
	// NextIssChain is the pending issuer certificate chain.
	NextIssChain []*x509.Certificate
	// NextIssKey is the pending issuer private key.
	NextIssKey any
}
...
// OptionsRenewX509 are the options for RenewX509.
type OptionsRenewX509 struct {
	// Existing is the current X.509 bundle. Its trust anchors and active
	// issuer pair are retained.
	Existing *X509
	// X509RootKey is a freshly generated key for the new self-signed root.
	// The old root key is never reused (and is never persisted).
	X509RootKey      crypto.Signer
	TrustDomain      string
	AllowedClockSkew time.Duration
	OverrideCATTL    *time.Duration
}

// RenewX509 generates a fresh self-signed root certificate and issuer pair,
// appends the new root to the existing trust anchors, and returns a copy of
// the existing bundle with the pending (next) issuer pair populated. The
// active issuer pair is unchanged: callers keep signing with it until the
// propagation grace period elapses and the next pair is promoted.
func RenewX509(opts OptionsRenewX509) (*X509, error) {
	log.Debugf("Renewing X.509 bundle with trust domain %s", opts.TrustDomain)

	next, err := GenerateX509(OptionsX509{
		X509RootKey:      opts.X509RootKey,
		TrustDomain:      opts.TrustDomain,
		AllowedClockSkew: opts.AllowedClockSkew,
		OverrideCATTL:    opts.OverrideCATTL,
	})
	if err != nil {
		return nil, err
	}

	trustAnchors := make([]byte, 0, len(opts.Existing.TrustAnchors)+len(next.TrustAnchors))
	trustAnchors = append(trustAnchors, opts.Existing.TrustAnchors...)
	trustAnchors = append(trustAnchors, next.TrustAnchors...)

	return &X509{
		TrustAnchors:    trustAnchors,
		IssChainPEM:     opts.Existing.IssChainPEM,
		IssKeyPEM:       opts.Existing.IssKeyPEM,
		IssChain:        opts.Existing.IssChain,
		IssKey:          opts.Existing.IssKey,
		NextIssChainPEM: next.IssChainPEM,
		NextIssKeyPEM:   next.IssKeyPEM,
		NextIssChain:    next.IssChain,
		NextIssKey:      next.IssKey,
	}, nil
}
