...
- [Workflow left permanently PENDING when a placement rebalance interrupted its start reminder registration during a Scheduler outage](#workflow-left-permanently-pending-when-a-placement-rebalance-interrupted-its-start-reminder-registration-during-a-scheduler-outage)
...
## Workflow left permanently PENDING when a placement rebalance interrupted its start reminder registration during a Scheduler outage

### Problem

Creating a workflow while the Scheduler was unreachable could leave the new instance permanently stuck in `PENDING` if a placement dissemination for the workflow actor type happened at the same time, for example the application scaling up or down during a Scheduler rollout.
The workflow actor commits its state first and then registers the `start-` reminder that launches execution, retrying the registration while the Scheduler is unavailable.
That retry ran under the placement claim context of the invocation, which a dissemination round cancels after the drain timeout regardless of the Scheduler's health, so the registration was abandoned with the state already committed.
The create returned an error, but the instance existed: it reported `PENDING` on every status read, had no reminder registered, never executed, and never transitioned to `FAILED`.

Retrying the create did not recover it either.
The retry checks whether the start reminder is missing before re-asserting it, and that check is a Scheduler round-trip that failed during the very outage that lost the reminder, so the retry aborted instead of re-asserting.
A client that reads the workflow status and skips the create while the instance is not terminal, the pattern used in the documentation examples, never issued another create at all, and nothing else ever touched the instance.

### Impact

You were affected if workflows were created, including re-created under a reused instance ID, while the Scheduler was disrupted and the application's pod count changed within a few seconds of the create.
A Scheduler outage alone did not cause it; the coincidence with a placement round did.
Affected instances were silently lost: callers polling them saw a healthy-looking `PENDING` status forever, and the instances could only be recovered by purging and recreating them.

### Root Cause

The start reminder registration performed after the state commit was bounded by the placement claim context of the invocation rather than by the lifetime of the runtime, so a dissemination round for the workflow actor type could abandon the only driver of an already committed instance.
The recovery path in the create retry depended on a Scheduler `Get` succeeding, and the status read used by clients bypassed the workflow actor entirely, so no code path re-asserted the missing reminder.

### Solution

Once the state is committed, a start or wake-up reminder registration that fails for any reason other than a permanent request error is handed to a detached retry on the runtime lifetime, so a cancelled claim context or an exhausted retry budget can no longer leave a committed instance without a driver while the sidecar runs.
The create still returns the error to the caller, and the registration completes as soon as the Scheduler is reachable again.
The create retry now re-asserts the start reminder whenever the reminder check cannot reach the Scheduler, instead of failing.
Finally, a status read of a `PENDING` instance whose start is overdue by more than 5 seconds re-asserts its start reminder, through both the status API and status waits, so an instance whose sidecar died before the detached retry completed is recovered by any client that polls it, without a new create.
...
A signed child created before this fix that has already continued as new no longer holds its creation input anywhere, so its completion still cannot be verified; such instances are the ones running across the upgrade.
