...
- [RabbitMQ pub/sub dead-lettered in-flight messages during graceful shutdown](#rabbitmq-pubsub-dead-lettered-in-flight-messages-during-graceful-shutdown)
...
## RabbitMQ pub/sub dead-lettered in-flight messages during graceful shutdown

### Problem

When the RabbitMQ pub/sub subscription handler returned an error because its context was cancelled, for example during Dapr's graceful shutdown, `handleMessage` unconditionally NACKed the message.

### Impact

You were affected if you ran the RabbitMQ pub/sub component with dead-lettering enabled (`enableDeadLetter: true`, the default) and `requeueInFailure: false` (also the default), and daprd was shut down, restarted, or scaled down while messages were in flight.
In-flight messages were routed to the dead-letter queue instead of being redelivered by RabbitMQ once the AMQP connection closed, appearing as message loss to the application even though RabbitMQ itself never dropped the message.

### Root Cause

`handleMessage` NACKed on every handler error without distinguishing a context cancellation, caused by the subscription shutting down, from a genuine application-level processing failure.
Both produced the same NACK-without-requeue outcome, sending the message straight to the dead-letter queue.

### Solution

`handleMessage` now checks the context error before NACKing.
When the context has been cancelled, the message is left unacknowledged instead of NACKed, so RabbitMQ redelivers it to another consumer once the connection closes rather than routing it to the dead-letter queue.
Handler errors unrelated to context cancellation continue to be NACKed as before. ([PR](https://github.com/dapr/components-contrib/pull/4450))
