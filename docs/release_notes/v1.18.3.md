...
The timeout is now configurable through the new daprd `--binding-options-timeout` flag, which applies to both the HTTP `OPTIONS` probe and the gRPC `ListInputBindings` probe.
