...
The Dapr releases are increasingly focusing on operational concerns like stability, reliability, and upgradability in production environments.
...
#### Serialization for State Transaction APIs have to be handled in component

Before v0.11.0, some of the components (eg: Redis, PostgresSQL) did not handle State Transaction Upsert request value serialization similar to how it was handled in the corresponding `Set` API path. Thus there was a change made in the runtime to convert byte array request values into string which was then passed on to be handled by the component. This caused an issue where properly serialized values were serialized again.

```go
// prior to 0.11.0 in state/redis/redis.go
b, _ := r.json.Marshal(req.Value)
```

From v0.11.0, the individual component needs to handle the serialization of the values properly, an example for the Redis state transaction upsert operation is:


```go
//defined in state/utils/utils.go
