...
- [Conversation Component AI Model Configuration Improvements](#conversation-component-ai-model-configuration-improvements)
...
## Centralized Model Management for Conversation Components

### Problem
Conversation providers had hardcoded AI model values, making it difficult to update models across environments or providers. This limited runtime flexibility and required rebuilding or redeploying when model changes were needed.

### Impact
Users could not easily change models per environment (dev, staging, prod) or override defaults at runtime. Infrastructure as code support for configuring models was limited.

### Root cause
AI model configuration was distributed across multiple providers with no centralized management or environment variable support.

### Solution
Introduced centralized model management with environment variable-driven defaults. All conversation providers now reference models from a single source (`conversation/models.go`). Models can be overridden using environment variables such as `OPENAI_MODEL`, `AZURE_OPENAI_MODEL`, and others. Documentation, examples, and tests were updated accordingly. This enables runtime flexibility, environment-specific configuration, and simpler future updates.
