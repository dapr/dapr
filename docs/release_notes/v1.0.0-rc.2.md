...
* Actors in Java SDK communicate to Dapr sidecar via GRPC. This allows better performance for invocation of Actor methods, timers and reminders. `ActorProxyBuilder` is not closeable, holding an instance of gRPC's `ManagedChannel` class to be closed. See [upgrade instructions for the Java SDK](https://github.com/dapr/java-sdk/issues/415).
