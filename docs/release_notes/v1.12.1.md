...
Azure EventHub component returns warning message says the storageAccountKey will be used but StorageAccountName is actually used in that condition.
...
The `NOGROUP` message is indicative of the consumer group no longer existing. This could be because the group or stream were manually deleted outside of Dapr or because the external Redis server was restarted/reconfigured.
