...
@adrianhr91, @alicejgibbons, @antontroshin, @aqeelat, @arslanmusta, @artur-ciocanu, @arturotrenard, @artursouza, @bhagya05, @cicoyle, @daixiang0, @DeepanshuA, @diegodocs, @divzi-p, @Druid-of-Luhn, @elena-kolevska, @emsearcy, @evhen14, @famarting, @filintod, @GowthamHN, @hhunter-ms, @humandigital-ruud, @ItalyPaleAle, @Jeroen-VdB, @jev-e, @JoshVanL, @jseguillon, @kaibocai, @LeonMatthesKDAB, @lor1113, @luigirende, @m3nax, @mahparaashley, @marcduiker, @mikeee, @MregXN, @msfussell, @passuied, @paule96, @paulyuk, @qustavo, @RafaelJCamara, @rochabr, @shubham1172, @Shubhdeep02, @sicoyle, @siri-varma, @SpiffyEight77, @Taction, @ThomasVitale, @tomhreb, @vil02, @WhitWaldo, @XavierGeerinck, @yaron2, @ytimocin, @znd4
...
Dapr's new conversation API reduces the complexity of interacting with large language models (LLM) at scale, providing one consistent API entry point to talk to underlying LLM providers. With the conversation API, you can:
- Enable critical performance and security functionality, like [prompt caching](https://v1-15.docs.dapr.io/developing-applications/building-blocks/conversation/conversation-overview/#prompt-caching) and [PII obfuscation](https://v1-15.docs.dapr.io/developing-applications/building-blocks/conversation/conversation-overview/#personally-identifiable-information-pii-obfuscation).
- Pair with Dapr resiliency policies and middleware for authentication and error handling.

Like with other APIs, Dapr provides observability by issuing metrics for your LLM interactions.

The Go, .NET, and Rust SDKs have the conversation API implemented as part of this release.

Learn more about the [conversation API](https://v1-15.docs.dapr.io/developing-applications/building-blocks/conversation/conversation-overview/), or dive into a [.NET or Go quickstart](PR needs merged).
...
Note: Prior to Dapr v1.15, actor reminders were run using the Placement service. Now, by default, the SchedulerReminders feature flag is set to true, and all new actor reminders you create are run using the Scheduler service to make them more scalable. When you deploy Dapr v1.15, *any existing actor reminders are migrated from the Placement service to the Scheduler service as a one time operation for each actor type*. You can prevent this migration by setting the SchedulerReminders flag to false in the application configuration file for the actor type.
...
### Python SDK
The Python SDK added support for [bidirectional streaming for pub/sub](https://v1-15.docs.dapr.io/developing-applications/sdks/python/python-client/#streaming-message-subscription) and [mock actors](https://v1-15.docs.dapr.io/developing-applications/sdks/python/python-actor/#mock-actor-testing) for unit testing. The Dapr Client workflow functions were deprecated in favor of the DaprWorkflowClient. Support for Python 3.8 was removed and support for Python 3.13 was added.
...
- **FIXED** fix(helm): Updated the default value of app.kubernetes.io/managed-by from "helm"… [8052](https://github.com/dapr/dapr/pull/8052)
- **FIXED** fix: add apiVersion and kind to volumeClaimTemplates [8067](https://github.com/dapr/dapr/pull/8067)
- **ADDED** feat(outbox): use content-type in the metadata for projections item [8149](https://github.com/dapr/dapr/pull/8149)
- **FIXED** docs: fix upgrade dapr with helm link [8162](https://github.com/dapr/dapr/pull/8162)
- **FIXED** fix: runtime outputs memory address on startup for metrics spec [8169](https://github.com/dapr/dapr/pull/8169)
- **ADDED** Enable PII scrubbing for LLM inputs/outputs [8189](https://github.com/dapr/dapr/issues/8189)
- **FIXED** fix(helm): Moved priority class declaration in the right position of dapr_placement_statefulset.yaml manifests [8205](https://github.com/dapr/dapr/pull/8205)
...
- **FIXED** Fixing e2e tests, podman and scheduler fail with mariner images [1450](https://github.com/dapr/cli/pull/1450)
- **FIXED** Fix a typo [1453](https://github.com/dapr/cli/pull/1453)
- **FIXED** fix: population of the schedulerhostaddress in self-hosted mode [1475](https://github.com/dapr/cli/pull/1475)
- **UPDATED** release: pin go to 1.23.5 [1477](https://github.com/dapr/cli/pull/1477)
### Components
