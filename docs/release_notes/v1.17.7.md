...
- Subscriptions occasionally tripped a `sync: WaitGroup is reused before previous Wait has returned` panic under load when contrib's retry loop raced the runtime's `wg.Wait`.

### Root Cause

Two compounding issues:

1. The runtime's `Subscription.Stop()` set its `closed` flag immediately on entry, which caused the handler closure to reject any further deliveries from contrib with `errors.New("subscription is closed")`.
...
2. The handler called `wg.Add(1)` *after* observing the closed flag, with no synchronization between the check and the Add.
   `Stop()` could observe inflight=0, start `wg.Wait`, and then race a contrib retry's `wg.Add(1)` — the Go runtime panics on `WaitGroup` reuse before `Wait` returns.
...
3. Polls inflight with a stable-quiet predicate (100 ms of consecutive zero readings on the paused path) so the drain does not seal in the sub-millisecond gap between handler return and the next claim-buffer read.
...
5. Seals the drain under a `sync.RWMutex` write lock so the handler's check-and-Add region is atomic with respect to `wg.Wait`, eliminating the `WaitGroup` reuse panic.
