...
## Fix Kafka losing messags when facing frequent rebalancing and/or deactivation of daprd

### Problem

In rare situations, the Kafka component could skip the processing lifecycle of a message when the underlying SDK session is cancelled, such as frequent consumer group rebalancing.

### Impact

Messages would potentially be skipped from processing.

### Root cause

The underlying handling of the Sarama library context cancellation was not allowing for messages to stop processing if the session is cancelled.

### Solution

The consumer processing logic was changed to check if the context is cancelled, and if so then immediately stop the processing of further messages.
