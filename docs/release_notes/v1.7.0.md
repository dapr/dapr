...
## Deprecation Notices
- Starting in Dapr 1.9, `content-type` will no longer default to `text/plain` if not explictly provided as a header for HTTP invocations. Users are recommended to turn on this new behavior via the new [preview feature](https://v1-7.docs.dapr.io/operations/support/support-preview-features/) to evaluate the impact of this change.
