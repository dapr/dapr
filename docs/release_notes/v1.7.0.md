...
## Deprecation Notices
- Content-type will not longer default to `application/json` if not explictly provided as a content-type header value for HTTP invocations starting in Dapr 1.9. Users are recommended to turn on the 
    - **FIXED** Allow disabling of default content-type for service invocation over HTTP where no content-type is specified. [4469](https://github.com/dapr/dapr/issues/4469)
