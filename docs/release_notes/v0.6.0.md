...
* Introduced *new* liveness/readiness health probe endpoint for Dapr sidecar
* Introduced *new* liveness/readiness health probe endpoint for actor service health API in user application https://github.com/dapr/docs/blob/master/reference/api/actors_api.md#health-check
...
**Updated** client validation of Sentry to require TLS certificiation all the time
...
For this release, we split the Dapr runtime binary (daprd.exe) from daprio/dapr image to reduce the size of sidecar image. You must confirm that you deploy Dapr to Kubernetes cluster using the latest 0.6.0 Dapr cli or helm chart.
