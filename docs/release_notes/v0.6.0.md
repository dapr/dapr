...
* Added mandatory TLS between the Dapr sidecar and the Dapr control plane
...
Since this release, we split the Dapr runtime binary from daprio/dapr image to reduce the size of sidecar image. You must confirm that you deploy Dapr to Kubernetes cluster using the latest 0.6.0 Dapr cli or helm chart.
