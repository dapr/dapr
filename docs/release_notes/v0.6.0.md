...
* Introduced *new* liveness/readiness probe endpoint for Dapr system/sidecar as well as actor service health API in User application
* Improved security by separating gRPC server for internal and external user app and enabling mTLS encryption on the connections between Dapr system components and runtime
* Supported global and granular pubsub scoping
* Added new components: Hazelcast pubsub, AliCloud Object storage binding, AWS Kinesis binding, and GCP Pubsub
* Introduced Dapr longhaul-test app and environment
* Improve e2e test coverage and release process for manifest yaml
...
> **Note: This release contains a few minor breakings. See [breaking changes](#breaking-changes).**
...
Actor service must expose /healthz endpoint and return 200. Otherwise, Dapr will treat actor service as unhealthy state. All actor user must upgrade sdk to the latest version.

### Separated Dapr sidecar image

Since this release, we split Dapr runtime binary from daprio/dapr image to reduce the size of sidecar image. You must confirm that you deploy Dapr to Kubernetes cluster using the latest 0.6.0 Dapr cli or helm chart.
