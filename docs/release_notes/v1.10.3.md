...
## Fixed error message lost

### Problem

Application using GRPC protocol returns an error message, the error message is lost and replaced with `message is nil`.

### Impact

This impacts users on Dapr 1.10.0-1.10.2, application using GRPC protocol and the endpoint returns an error message.

### Root cause

For HTTP api application response setting to `replayableRequest` was ignored by mistake. For GRPC api, the error message is replaced with `message is nil` when checking the response body.

### Solution

We fix the check of response for both http and grpc api.
